/*
  DESCRIPTION:
  CentOS Stream 10 unified storage variables for Packer Plugin for Proxmox (proxmox-iso)
*/
...
variable "vm_storage" {
  ...
  validation {
    ...
    error_message = "Partition role must be one of: efi, bios_grub, pv, swap, filesystem."
  }
  ...
  validation {
    condition = alltrue([
      ...
      contains(["ext4","xfs","fat32","swap","bios_grub",""], p.filesystem.fstype)
    ])
    error_message = "Filesystem type must be ext4, xfs, fat32, swap, bios_grub, or empty."
  }
  ...
  validation {
    condition = alltrue(flatten([
      for disk in var.vm_storage.disks : [
        for p in disk.partitions :
        !(try(p.filesystem.fstype, "") == "bios_grub" && p.size > 2)
      ]
    ]))

    error_message = "BIOS_GRUB partitions must be 2MB or smaller."
  }
